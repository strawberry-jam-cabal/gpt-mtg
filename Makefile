...
build-card: ## Build the container
	docker build -t mtg-gpt2-card -f dev/docker/card/Dockerfile .
...
run-card: ## Run the container
	docker run --rm -v $(shell pwd):$(shell pwd) -it -p 8989:8989 mtg-gpt2-card:latest
